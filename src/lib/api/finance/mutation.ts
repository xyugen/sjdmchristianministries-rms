...
import { TransactionCategory, TransactionType } from "@/constants/transaction";
...
export const editFinancialTransaction = async (id: string, transaction: EditFinancialTransaction) => {
  try {
    return await db.update(financialTransactionsTable)
      .set(transaction)
...
export const deleteFinancialTransaction = async (id: string) => {
  try {
    return await db.delete(financialTransactionsTable)
      .where(eq(financialTransactionsTable.id, id))
      .returning();
  } catch (error) { 
      console.log(error);
